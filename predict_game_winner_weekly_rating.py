...
def concat_seasons():
    ...
    for season in seasons:
        ...
        try:
            season_df = pd.read_csv(f'{currdir}/MRegularSeasonDetailedResults_{season}_weekly_matchups.csv')
...
def fit_model_scalar(model_param, model_name, X, y):
    ...
    for train_index, test_index in tscv.split(X):
        ...
        accuracies.append(accuracy_score(y_test, y_pred))
    avg_acc = np.mean(accuracies)
    print(f'{model_name} scalar accuracy: {avg_acc:.2f}')

if __name__ == '__main__':
    # df = pd.read_csv('data/Mens/Season/2022/MRegularSeasonDetailedResults_2022_weekly_matchups.csv')
    df = concat_seasons()
