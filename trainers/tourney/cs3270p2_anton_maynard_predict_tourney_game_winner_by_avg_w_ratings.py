...
def concat_seasons():
    ...
    for season in seasons:
        ...
        try:
            season_df = pd.read_csv(f'{currdir}/MNCAATourneyDetailedResults_{season}_tourney_avg_w_rating.csv')
...
def fit_model_scalar(model_param, model_name, x, y):
    ...
    for train_index, test_index in tscv.split(x):
        ...
        top_accuracy = accuracy_score(y_test[:50], y_pred[:50])
        bottom_accuracy = accuracy_score(y_test[-50:], y_pred[-50:])
        print(f'Log Regression Top 50 accuracy: {top_accuracy:.5f}, Bottom 50 accuracy: {bottom_accuracy:.5f}')
...
def timeseriessplit_by_season(model, model_name, seasons_data):
    ...
    for i in range(1, len(seasons_data)):
        ...
        top_accuracy = accuracy_score(y_test[:200], y_pred[:200])
        bottom_accuracy = accuracy_score(y_test[-200:], y_pred[-200:])
        print(f'Log Regression Season {seasons_data[i]["Season"].unique()[0]} Top 200 accuracy: {top_accuracy:.5f}, Bottom 200 accuracy: {bottom_accuracy:.5f}')
