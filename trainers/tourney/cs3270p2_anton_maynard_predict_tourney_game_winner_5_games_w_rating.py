    ...
    for season in seasons:
        ...
        try:
            season_df = pd.read_csv(
                f'{curr_dir}/MNCAATourneyDetailedResults_{season}_tourney_avg_5_w_rating.csv')
            all_seasons = pd.concat([all_seasons, season_df])
        except FileNotFoundError as error:
            print(f'Error processing {season}: {error}')
            pass
    return all_seasons
...
if __name__ == '__main__':
    ...
    print(df.columns)
    X = df.drop(
        ['Season', 'DayNum', 'team_1', 'team_1_DayNum',
         'team_1_Week_x', 'team_2', 'team_2_DayNum',
