...
def concat_seasons():
    ...
    for season in seasons:
        ...
        try:
            season_df = pd.read_csv(f'{currdir}/MNCAATourneyDetailedResults_{season}_tourney_avg_5_games.csv')
...
def timeseriessplit_by_season(model, model_name, seasons_data):
    ...
    for i in range(1, len(seasons_data)):
        ...
        x_train = train.drop(['Season', 'DayNum', 'team_1_DayNum', 'team_1_Week', 'team_2_DayNum', 'team_2_Week', 'team_1_won'], axis=1)
        y_train = train['team_1_won']
        x_train = scaler.fit_transform(x_train)
        test = seasons_data[i]
        test = test.dropna(axis='columns', how='any')
        x_test = test.drop(['Season', 'DayNum', 'team_1_DayNum', 'team_1_Week', 'team_2_DayNum', 'team_2_Week', 'team_1_won'], axis=1)
        x_test = scaler.transform(x_test)
        y_test = test['team_1_won']
        model.fit(x_train, y_train)
        y_pred = model.predict(x_test)
        mid = int(len(y_test)/2)
        top_accuracy = accuracy_score(y_test[:mid], y_pred[:mid])
        bottom_accuracy = accuracy_score(y_test[-mid:], y_pred[-mid:])
        print(f'Log Regression Season {seasons_data[i]["Season"].unique()[0]} Top Half accuracy: {top_accuracy:.5f}, Bottom Half accuracy: {bottom_accuracy:.5f}')
...
if __name__ == '__main__':
    ...
    x = df.drop(['Season', 'DayNum', 'team_1', 'team_1_DayNum', 'team_1_Week', 'team_2', 'team_2_DayNum', 'team_2_Week', 'team_1_won'], axis=1)
    y = df['team_1_won']
    classifiers = {
