...
def concat_stats_variants():
    ...
    data_variants = {}
    data_variants['avg_5_df'] = pd.read_csv(f'data/Mens/Season/{SEASON}/MRegularSeasonDetailedResults_{SEASON}_avg_5_games.csv').groupby('TeamID').last().reset_index()
    ...
    data_variants['rating_rol5_df'] = pd.read_csv(f'data/Mens/Season/{SEASON}/MRegularSeasonDetailedResults_{SEASON}_avg_5_w_rating.csv').groupby('TeamID').last().reset_index()
...
if __name__ == '__main__':
    ...
    avg_5_models = trained_models['avg_5']
    averaged_10_data = {'data': data['avg_5_df'],'drop_columns': ['team_1_DayNum', 'team_1_Week','team_2_DayNum', 'team_2_Week']}
    tournament_preds['avg_5'] = []
    tourney_seeds['avg_5'] = []
    for model in avg_5_models.values():
        tournament, seeds = predict_tourney_bracket(averaged_10_data, model)
        tournament_preds['avg_5'].append({type(model).__name__ : tournament})
        tourney_seeds['avg_5'].append({type(model).__name__ : seeds})
    ...
    rating_rol5_models = trained_models['rating_rol5']
    rating_roll5_data = {'data': data['rating_rol5_df'],'drop_columns': [ 'team_1_DayNum',
             'team_1_Week_x', 'team_1_Week_y', 'team_2_DayNum',
             'team_2_Week_x','team_2_Week_y',]}
   
    tournament_preds['rating_rol5'] = []
    tourney_seeds['rating_rol5'] = []
    for model in rating_rol5_models.values():
        tournament, seeds = predict_tourney_bracket(rating_roll5_data, model)
        tournament_preds['rating_rol5'].append({type(model).__name__ : tournament})
        tourney_seeds['rating_rol5'].append({type(model).__name__ : seeds})
    ...
    t_avg_models = trained_models['tourney_avg']
    t_avg_data = {'data': data['avg_df'],'drop_columns': []}
    tournament_preds['tourney_avg'] = []
    tourney_seeds['tourney_avg'] = []
    for model in t_avg_models.values():
        tournament, seeds = predict_tourney_bracket(t_avg_data, model)
        tournament_preds['tourney_avg'].append({type(model).__name__ : tournament})
        tourney_seeds['tourney_avg'].append({type(model).__name__ : seeds})

    t_avg_5_models = trained_models['tourney_avg_5']
    t_avg_5_data = {'data': data['avg_5_df'],'drop_columns': ['team_1_DayNum', 'team_1_Week','team_2_DayNum', 'team_2_Week']}
    tournament_preds['tourney_avg_5'] = []
    tourney_seeds['tourney_avg_5'] = []
    for model in t_avg_5_models.values():
        tournament, seeds = predict_tourney_bracket(t_avg_5_data, model)
        tournament_preds['tourney_avg_5'].append({type(model).__name__ : tournament})
        tourney_seeds['tourney_avg_5'].append({type(model).__name__ : seeds})

    t_avg_rating_models = trained_models['tourney_avg_rating']
    t_avg_rating_data = {'data': data['avg_rating_df'],'drop_columns': []}
    tournament_preds['tourney_avg_rating'] = []
    tourney_seeds['tourney_avg_rating'] = []
    for model in t_avg_rating_models.values():
        tournament, seeds = predict_tourney_bracket(t_avg_rating_data, model)
        tournament_preds['tourney_avg_rating'].append({type(model).__name__ : tournament})
        tourney_seeds['tourney_avg_rating'].append({type(model).__name__ : seeds})
    
    t_rating_models = trained_models['tourney_rating_rol5']
    t_rating_data = {'data': data['rating_rol5_df'],'drop_columns': [ 'team_1_DayNum',
             'team_1_Week_x', 'team_1_Week_y', 'team_2_DayNum',
             'team_2_Week_x','team_2_Week_y',]}
    tournament_preds['tourney_rating_rol5'] = []
    tourney_seeds['tourney_rating_rol5'] = []
    for model in t_rating_models.values():
        tournament, seeds = predict_tourney_bracket(t_rating_data, model)
        tournament_preds['tourney_rating_rol5'].append({type(model).__name__ : tournament})
        tourney_seeds['tourney_rating_rol5'].append({type(model).__name__ : seeds})
