...
def prepare_team_stats(df):
    ...
    stats_rolling = all_stats.groupby(['Season','TeamID']).rolling(window=5, min_periods=1).mean().reset_index(drop=True)
    against_rolling = all_against_stats.groupby(['Season','TeamID']).rolling(window=5, min_periods=1).mean().reset_index(drop=True)
...
def main():
    ...
    for season in weekly_stats_df['Season'].unique():
        ...
        season_stats.to_csv(f'{data_location}/{season}/MRegularSeasonDetailedResults_{season}_avg_5_w_rating.csv', index=False)
        prepared_matches = prepare_matchup_data(season_games, season_stats)
        prepared_matches.to_csv(f'{data_location}/{season}/MRegularSeasonDetailedResults_{season}_matchups_avg_5_w_rating.csv', index=False)
