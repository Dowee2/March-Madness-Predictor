...
def main():
    ...
    for season in range(2010, 2024):
        try:
            teams_df = pd.read_csv(f'{data_location}/{season}/MRegularSeasonDetailedResults_{season}_avg_w_rating.csv').groupby('TeamID').last().reset_index()
            ...
            matchup_data.to_csv(f'{data_location}/{season}/MNCAATourneyDetailedResults_{season}_tourney_avg_w_rating.csv', index=False)
